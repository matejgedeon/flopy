...
class MthSft(Package):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if unitnumber is None:
            unitnumber = MthSft._defaultunit()
        elif unitnumber == 0:
            unitnumber = MthSft._reservedunit()
    ...
    def load(
        ...
        dtype = MthSft.get_default_dtype(ncomp)
        ...
        if ext_unit_dict is not None:
            unitnumber, filenames[0] = model.get_ext_dict_attr(
                ext_unit_dict, filetype=MthSft._ftype()
            )
            if abs(ioutobs) > 0:
