...
class MthSsm(Package):
    ...
    crch : Transient2d, scalar, array of floats, or dictionary
        ...
        the ssm object.
    iSpec_rch : Transient2d, scalar, array of floats, or dictionary
        iSpec_RCH is the iSpecies distribution of recharge for MCCOMP species.
        If iSpec_rch changes by stress period, then the user must provide a
        dictionary, where the key is the stress period number (zero based) and
        the value is the iSpecies recharge array.
    ...
    iSpec_evt : Transient2d, scalar, array of floats, or dictionary
        iSpec_EVT is the iSpecies distribution of evapotranspiration.
        If iSpec_evt changes by stress period, then the user must provide a
        dictionary, where the key is the stress period number (zero based) and
        the value is the iSpecies evapotranspiration array.
    stress_period_data : dictionary
        ...
        ((cssms(n), float), n=1, ncomp)],("issmmc", int))
        Note the additional issmmc variable for the MTHP specifying the 
        solution number of the corresponding source solution. The solution
        number refers to the solution composition in the input for the
        geochemical solver with a number equal to ISSMMC + BASENR.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if unitnumber is None:
            unitnumber = MthSsm._defaultunit()
        elif unitnumber == 0:
            unitnumber = MthSsm._reservedunit()
        ...
        # check if an input for rch is needed (rch in modflow simulation) 
        self.crch = None
        try:
            ...
            if model.verbose:
                print("   explicit crch in file")
                
        self.iSpec_rch = None
        try:
            if iSpec_rch is None and model.mf.rch is not None:
                if model.mccomp > 0:
                    print("found 'rch' in modflow model, resetting iSpec_rch to 0.0")
                    iSpec_rch = 0.0
        except:
            if model.verbose:
                print("   explicit iSpec_rch in file")    

       # Note: list is used for multi-species & iSolution, NOT for stress periods!
       # If ncomp <= 0, crch becomes None
        if crch is not None and model.ncomp > 0:
            self.crch = []
            t2d = Transient2d(
                model,
                (nrow, ncol),
            ...
            if model.verbose:
                print("   crch not used because NCOMP <= 0")
        
        if iSpec_rch is not None and mccomp > 0:
            self.iSpec_rch = Transient2d(
                model,
                (nrow, ncol),
                int,
                iSpec_rch,
                name="iSpecies for recharge",
                locat=self.unit_number[0],
                array_free_format=False,
            )         
        else:
            if model.verbose:
                print("   iSpec_rch not used because MCCOMP <= 0")
                
        self.cevt = None
        try:
            if cevt is None and model.mf.evt is not None:
                if model.ncomp > 0:
                    print("found 'evt' in modflow model, resetting cevt to 0.0")
                    cevt = 0.0
        except:
            if model.verbose:
               print("   explicit crch in file")
        ...
        self.iSpec_evt = None
        try:
           if iSpec_evt is None and model.mf.evt is not None:
                if mccomp > 0:
                    print("found 'evt' in modflow model, resetting iSpec_evt to 0.0")
                    iSpec_evt = 0.0
        except:
            if model.verbose:
                print("   explicit iSpec_evt in file")

        if cevt is not None and ncomp > 0:
            self.cevt = []
            t2d = Transient2d(
                model,
                (nrow, ncol),
        ...
        if iSpec_evt is not None and mccomp > 0:
            self.iSpec_evt = Transient2d(
                model,
                (nrow, ncol),
                int,
                iSpec_evt,
                name="iSpecies for evapotransporation",
                locat=self.unit_number[0],
                array_free_format=False,
            )  
        else:
            if model.verbose:
                print(    "iSpec_evt not used because MCCOMP <= 0")
    ...
    def write_file(self):
        ...
        for kper in range(nper):
            ...
            if self.crch is not None or self.iSpec_rch is not None:
                # Determine if anything needs to be written
                incrch = -1
                ...
                if self.crch is not None:
                    for t2d in self.crch:
                        ...
                        if incrch == 1:
                            break
                if self.iSpec_rch is not None:
                    inisrchicomp, file_entry = self.iSpec_rch.get_kper_entry(kper)
                    incrch = max(incrch, inisrchicomp)
                ...
                if incrch == 1:
                    ...
                    if self.iSpec_rch is not None:        
                        u2d = self.iSpec_rch[kper]
                        file_entry = u2d.get_file_entry()
                        f_ssm.write(file_entry)
            
            if self.cevt is not None or self.iSpec_evt is not None:
                incevt = -1
                if self.cevt is not None:
                    for t2d in self.cevt:
                        ...
                        if incevt == 1:
                            break
                if self.iSpec_evt is not None:
                    inisevticomp, file_entry = self.iSpec_evt.get_kper_entry(kper)
                    incevt = max(incevt, inisevticomp)
    ...
    def load(
        ...
        # dtype
        dtype = MthSsm.get_default_dtype(ncomp)
        ...
        for iper in range(nper):
            ...
            if incrch >= 0:
                ...
                if mccomp > 0:
                    if model.verbose:
                        print("   loading iSpec for recharge...")
                    t = Util2d.load(
                        f,
                        model,
                        (nrow,ncol),
                        int,
                        "iSpecies for recharge",
                        ext_unit_dict,
                        array_format="mt3d",
                        )
            ...
            if incevt >= 0:
                ...
                if mccomp > 0:
                    if model.verbose:
                        print("   loading iSpec for evapotranspiration...")
                    t = Util2d.load(
                        f,
                        model,
                        (nrow,ncol),
                        int,
                        "iSpecies for evapotranspiration",
                        ext_unit_dict,
                        array_format="mt3d",
                        )
            ...
            # Item D8: KSS, ISS, JSS, CSS, ITYPE, (CSSMS(n),n=1,NCOMP,ISSMMC)
            if model.verbose:
                print(
                    "   loading KSS, ISS, JSS, CSS, ITYPE, "
                    "(CSSMS(n),n=1,NCOMP), ISSMMC..."
                )
            if nss > 0:
        ...
        if ext_unit_dict is not None:
            unitnumber, filenames[0] = model.get_ext_dict_attr(
                ext_unit_dict, filetype=MthSsm._ftype()
            )
